...
### Input Form
![Sleep Disorder Predictor Form](screenshots/vid.mov)
...
- Predicts sleep disorder category:
  ...
  - `Insomnia`
  - `Sleep Apnea`
- `/predict` → fast prediction only  
- `/predict_with_explanation` → prediction + SHAP + LLM explanation  
...
├── .env.example
│
├── artifacts/             # Generated by train.py (ignored by git)
...
- Docs → `/docs`
- Predict → `/predict`
- Predict + Explain → `/predict_with_explanation`
